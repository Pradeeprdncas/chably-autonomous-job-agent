...
export const endpoints = {
  ...
  saveJob: (jobId, userId, status = 'saved') => api(`/api/v1/jobs/${encodeURIComponent(jobId)}/save`, { method: 'POST', body: JSON.stringify({ user_id: userId, status, notes: '' }) }),
  createApplication: (opportunityId, userId, status = 'saved') => api(`/api/v1/opportunities/${encodeURIComponent(opportunityId)}/application?user_id=${encodeURIComponent(userId)}`, { method: 'POST', body: JSON.stringify({ status, notes: '' }) }),
  ...
  gmailThreads: (folder = 'all') => api(`/api/v1/gmail/threads?folder=${encodeURIComponent(folder)}&limit=50&offset=0`),
  gmailThread: (threadId) => api(`/api/v1/gmail/threads/${encodeURIComponent(threadId)}`),
